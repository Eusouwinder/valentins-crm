...
// WhatsApp Calling API webhook types
interface MetaWebhookCallValue {
...
interface MetaWebhookCallChange {
  value: MetaWebhookCallValue;
  field: "calls";
}
...
Deno.serve(async (req) => {
  ...
  if (firstChange.field === "calls") {
    return await handleCallWebhook(
      supabase,
      channel as { id: string; organization_id: string; credentials: Record<string, unknown>; settings: Record<string, unknown> },
      channelId,
      firstChange.value as unknown as MetaWebhookCallValue,
      payload
    );
  }
  ...
  try {
    ...
    // Process call permission replies (interactive messages)
    if (value.messages?.[0]?.type === "interactive") {
      const interactiveMsg = value.messages[0];
      const interactiveData = interactiveMsg.interactive as Record<string, unknown> | undefined;
      if (interactiveData?.type === "call_permission_reply") {
        const reply = interactiveData as { type: string; call_permission_reply?: { response: string } };
        await handleCallPermissionReply(
          supabase,
          channel.organization_id,
          channelId,
          interactiveMsg.from,
          reply.call_permission_reply?.response || "UNKNOWN"
        );
      }
    }
...
/**
 * Handle WhatsApp Calling API webhook events (field: "calls").
 * Events: connect (call answered / incoming call), terminate (call ended).
 */
async function handleCallWebhook(
  ...
  const callId = callValue.call_id;
  const event = callValue.event;

  // Dedup
  const externalEventId = `call_${event}_${callId}`;
  const { error: eventInsertErr } = await supabase
    .from("messaging_webhook_events")
    .insert({
      channel_id: channelId,
  ...
  if (eventInsertErr?.message?.toLowerCase().includes("duplicate")) {
    console.log(`[Webhook/Call] Duplicate event ignored: ${externalEventId}`);
    return json(200, { ok: true, duplicate: true });
  }

  try {
    if (event === "connect") {
      await handleCallConnect(supabase, channel, callValue);
    } else if (event === "terminate") {
      await handleCallTerminate(supabase, channel, callValue);
    } else {
      console.log(`[Webhook/Call] Unknown call event: ${event}`);
    }
    ...
    return json(200, { ok: true, event_type: `call_${event}` });
  } catch (error) {
    console.error("[Webhook/Call] Processing error:", error);
    return json(200, {
      ok: false,
      error: "Processing error",
      details: error instanceof Error ? error.message : "Unknown error",
...
/**
 * Handle "connect" call event.
 * - Outbound: Lead answered — update with SDP answer.
 * - Inbound: Lead is calling — insert new call row with SDP offer.
 */
async function handleCallConnect(
  supabase: ReturnType<typeof createClient>,
  ...
  if (direction === "outbound") {
    ...
    if (error) {
      console.error("[Webhook/Call] Error updating outbound call:", error);
    }

    // Broadcast to browser
    await broadcastCallEvent(supabase, channel.organization_id, {
      type: "call_answered",
      callId: call_id,
      sdpAnswer: sdp,
      direction: "outbound",
    ...
    // Inbound call — insert new row
    const { error } = await supabase.from("whatsapp_calls").insert({
      organization_id: channel.organization_id,
      wa_call_id: call_id,
      channel_id: channel.id,
      direction: "inbound",
    ...
    if (error) {
      console.error("[Webhook/Call] Error inserting inbound call:", error);
    }
...
/**
 * Handle "terminate" call event.
 * Updates call status and calculates duration.
 */
async function handleCallTerminate(
  supabase: ReturnType<typeof createClient>,
  channel: { id: string; organization_id: string },
  callValue: MetaWebhookCallValue
) {
  const { call_id, disconnect_reason } = callValue;
  const now = new Date().toISOString();

  // Determine final status based on disconnect reason
  let finalStatus = "completed";
  if (disconnect_reason === "rejected") finalStatus = "rejected";
  else if (disconnect_reason === "missed" || disconnect_reason === "no_answer") finalStatus = "missed";
  else if (disconnect_reason === "error" || disconnect_reason === "failed") finalStatus = "failed";

  // Get the call to calculate duration
  const { data: call } = await supabase
    .from("whatsapp_calls")
    .select("id, answered_at, voice_call_id")
    .eq("wa_call_id", call_id)
    .maybeSingle();

  let durationSeconds: number | null = null;
  if (call?.answered_at) {
    durationSeconds = Math.round(
      (new Date(now).getTime() - new Date(call.answered_at).getTime()) / 1000
    );
  }
  ...
  if (error) {
    console.error("[Webhook/Call] Error updating call terminate:", error);
  }
...
/**
 * Handle call permission reply (ACCEPT/DECLINE) from interactive message.
 */
async function handleCallPermissionReply(
  ...
  const { data: contact } = await supabase
    .from("contacts")
    .select("id, call_permission_status")
  ...
  if (!contact) {
    console.log(`[Webhook/Call] No contact found for permission reply from ${phone}`);
    return;
  }

  // Update contact's call_permission_status
  const permissionStatus = (contact.call_permission_status || {}) as Record<string, unknown>;
  permissionStatus[channelId] = {
    status: isAccepted ? "granted" : "declined",
    respondedAt: now,
    grantedAt: isAccepted ? now : null,
    expiresAt: isAccepted
  ...
  const { error } = await supabase
    .from("contacts")
    .update({ call_permission_status: permissionStatus })
    .eq("id", contact.id);

  if (error) {
    console.error("[Webhook/Call] Error updating call permission:", error);
  }

  console.log(`[Webhook/Call] Permission ${response} from ${phone} for channel ${channelId}`);

  // Broadcast permission update
  await broadcastCallEvent(supabase, organizationId, {
...
/**
 * Broadcast a call event to the browser via internal API.
 * Uses Supabase Broadcast channel: org:{orgId}:wa-calls
 */
async function broadcastCallEvent(
  ...
  if (!appUrl || !internalSecret) {
    console.log("[Webhook/Call] CRM_APP_URL or INTERNAL_API_SECRET not set, skipping broadcast");
    return;
  }

  try {
    const response = await fetch(`${appUrl}/api/voice/whatsapp-call/notify`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Internal-Secret": internalSecret,
    ...
    if (!response.ok) {
      console.error(`[Webhook/Call] Broadcast failed: ${response.status}`);
    }
  } catch (error) {
    console.error("[Webhook/Call] Broadcast error:", error);
  }
}
